...
def run_kyc_dashboard():
    ...
    # --- 2. DATABASE SESSION (V33) ---
    if 'db_kyc_v33' not in st.session_state:
        st.session_state.db_kyc_v33 = {
            thn: { kat: { c: {
    ...
    with tab_v:
        db_ref = st.session_state.db_kyc_v33[thn_v][kat_v]
    ...
    with tab_p:
        ...
        with st.container(border=True):
            ...
            old_val = st.session_state.db_kyc_v33[thn_v][kat_v][u_cbg]['r'][bln_v]
            ...
            if st.button("💾 Simpan Progres", use_container_width=True):
                st.session_state.db_kyc_v33[thn_v][kat_v][u_cbg]['r'][bln_v] = int(u_val) if u_val is not None else 0
                st.toast("Data Disimpan!")
                time.sleep(0.5)
                st.rerun()

    # --- TAB 3: TARGET PER RISK ---
    with tab_t:
        st.markdown("### 🎯 Setup Target Risiko")
        with st.form("form_target_v33"):
            l1, l2, l3, l4 = st.columns([1.5, 1, 1, 1])
            l1.markdown("**Cabang**")
            l2.markdown("🔴 **High**", help="Center Aligned")
            l3.markdown("🟡 **Med**")
            l4.markdown("🟢 **Low**")
            st.divider()
            ...
            for cbg in list_cabang:
                ...
                c1.write(f"**{cbg}**")
                curr = st.session_state.db_kyc_v33[thn_v][kat_v][cbg]['t']
                
                # Angka 0 dihapus otomatis (Blank)
                ...
                st.session_state.db_kyc_v33[thn_v][kat_v][cbg]['t']['High'] = val_h if val_h is not None else 0
                st.session_state.db_kyc_v33[thn_v][kat_v][cbg]['t']['Medium'] = val_m if val_m is not None else 0
                st.session_state.db_kyc_v33[thn_v][kat_v][cbg]['t']['Low'] = val_l if val_l is not None else 0
