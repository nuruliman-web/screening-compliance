...
def run_kyc_dashboard():
    ...
    # --- 2. DATABASE SESSION (V12) ---
    if 'db_kyc_v12' not in st.session_state:
        st.session_state.db_kyc_v12 = {}
    ...
    if thn_v not in st.session_state.db_kyc_v12:
        st.session_state.db_kyc_v12[thn_v] = {
    ...
    with tab_view:
        data_sumber = st.session_state.db_kyc_v12[thn_v][kat_v]
    ...
    with tab_input_progres:
        ...
        with st.form("form_real_v12", clear_on_submit=True):
            ...
            if st.form_submit_button("✅ Simpan Progres"):
                st.session_state.db_kyc_v12[thn_v][kat_v][in_cbg]['r'][in_bln] += int(in_val)
                # NOTIFIKASI BERHASIL
                st.success(f"Berhasil! Data {in_cbg} bulan {in_bln} telah ditambahkan.")
    ...
    with tab_input_target:
        ...
        with st.form("form_target_v12"):
            tcols = st.columns(4)
            for i, cbg in enumerate(list_cabang):
                cur_t = st.session_state.db_kyc_v12[thn_v][kat_v][cbg]['t']
                new_t = tcols[i % 4].number_input(f"Target {cbg}", min_value=0, value=cur_t)
                st.session_state.db_kyc_v12[thn_v][kat_v][cbg]['t'] = new_t
