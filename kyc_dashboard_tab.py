...
def run_kyc_dashboard():
    ...
    # --- 2. DATABASE SESSION (V13) ---
    if 'db_kyc_v13' not in st.session_state:
        st.session_state.db_kyc_v13 = {}
    ...
    if thn_v not in st.session_state.db_kyc_v13:
        st.session_state.db_kyc_v13[thn_v] = {
    ...
    with tab_view:
        data_sumber = st.session_state.db_kyc_v13[thn_v][kat_v]
    ...
    with tab_input_progres:
        ...
        with st.form("form_real_v13", clear_on_submit=True):
            col1, col2, col3 = st.columns(3)
            in_bln = col1.selectbox("Bulan:", list_bulan)
            in_cbg = col2.selectbox("Cabang:", list_cabang)
            # Pakai value=None supaya kotak input kosong (nggak ada angka 0)
            ...
            if st.form_submit_button("✅ Simpan Progres"):
                if in_val is not None:
                    st.session_state.db_kyc_v13[thn_v][kat_v][in_cbg]['r'][in_bln] += int(in_val)
                    st.success(f"Berhasil! Data {in_cbg} telah ditambahkan.")
                    st.toast(f"Data {in_cbg} Terupdate!", icon='✅')
                    time.sleep(1)
                    st.rerun()
                else:
                    st.error("Silakan isi angka dulu, Man!")

    # --- TAB 3: INPUT TARGET (FIX NO-ZERO) ---
    with tab_input_target:
        st.subheader(f"⚙️ Setting Target {kat_v} - {thn_v}")
        with st.form("form_target_v13"):
            tcols = st.columns(4)
            for i, cbg in enumerate(list_cabang):
                cur_t = st.session_state.db_kyc_v13[thn_v][kat_v][cbg]['t']
                # Target tahunan pakai value=cur_t (karena biasanya sudah ada angka sebelumnya), 
                # tapi kalau mau diganti tinggal blok/ketik.
                new_t = tcols[i % 4].number_input(f"Target {cbg}", min_value=0, value=cur_t, step=1)
                st.session_state.db_kyc_v13[thn_v][kat_v][cbg]['t'] = new_t
